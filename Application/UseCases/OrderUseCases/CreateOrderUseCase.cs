...
namespace Application.UseCases.OrderUseCases
{
    public class CreateOrderUseCase
    {
        ...
        public CreateOrderUseCase(IOrderCommand command, IDishQuery dishQuery, IDeliveryTypeQuery deliveryQuery)
        {
            _command = command;
        ...
        public async Task<OrderCreateResponse> ExecuteAsync(OrderRequest request)
        {
            ...
            if (!await _deliveryQuery.ExistsAsync(request.Delivery.Id))
                throw new BadRequestException400("Debe especificar un tipo de entrega válido");
            ...
            var orderCreated = await _command.InsertAsync(order);
